...
export default function AirtimePie() {
  const theme = useTheme();
  const [post, setPost] = React.useState(null);
 
  React.useEffect(() => {
    axios.get('/user/billschart').then((response) => {
      setPost(response);     
    });
  }, []);
  ...
  const chart = JSON.stringify(post.data.data.electricity);
  ...
  return (
    <Card>
      ...
      <CardHeader title="Electricity Bills Statistics" />
