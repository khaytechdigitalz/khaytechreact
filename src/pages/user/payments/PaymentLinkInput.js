...
export default function RequestPaymentInput() {
  ...
  const onSubmit = async (event, formState) => {
    try { 
      ...
      axios.post('/user/request/paymentlink', formData,{ 
        headers: {
  ...
  const gettotal = () => {
    try {
     ...
     const fee = general.plink_fee/100*amount;
    ...
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)} enctype="multipart/form-data">
      <Stack spacing={2.5}>
        ...
        <TextField type="number" name="amount" id="amount"  onKeyUp={gettotal} fullWidth label="Amount" />
          ...
          Please enter '0' in the amount field above if you want your customer to be able to enter any amount to pay
         </Typography>
       </Stack>
      ...
      <Stack alignItems="center" spacing={1}>
        ...
        <Typography variant="caption" sx={{ color: 'text.secondary', textAlign: 'center' }}>
          By clicking on the Create Button below, <br/>an payment link will be created which you can use to collect payment as a merchant from your customers or as a donation
        </Typography>
      </Stack>
